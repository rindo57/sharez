function showDirectory(data) {
    ...
    for (const [key, item] of files) {
        if (item.type === 'file') {
            ...
            html += `<tr data-path="${item.path}" data-id="${item.id}" data-name="${item.name}" class="body-tr file-tr"><td><div class="td-align"><img src="static/assets/file-icon.svg">${item.name}</div></td><td><div class="td-align">${size}</div></td><td><div class="td-align"><a href="/file?path=${item.path}/${item.id}" class="download-btn"><img src="static/assets/download-icon.svg" alt="Download"></a></div></td><td><div class="td-align"><a data-id="${item.id}" class="more-btn"><img src="static/assets/more-icon.svg" class="rotate-90"></a></div></td></tr>`
